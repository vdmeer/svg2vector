...
import de.vandermeer.svg2vector.applications.base.TmpArtefacts;
...
public class Svg2Vector_IS extends AppBase<IsLoader> {
	...
	protected final AO_InkscapeExecutable optionInkscapeExec = new AO_InkscapeExecutable('x');

	/** Application option to set DPI for bitmap export. */
	protected final AO_ExportDpi optionExpDpi = new AO_ExportDpi(SvgTargets.png, "--export-dpi");

	/** Application option to set PS level for PS export. */
	protected final AO_ExportPsLevel optionExpPslevel = new AO_ExportPsLevel(SvgTargets.ps, "--export-ps-level");

	/** Application option to set PDF version for PDF export. */
	protected final AO_ExportPdfVersion optionExpPdfver = new AO_ExportPdfVersion(SvgTargets.pdf, "--export-pdf-version");

	/** Application option to require SVG transformation first, then do the actual target transformation. */
	protected final AO_SvgFirst optionSvgFirst = new AO_SvgFirst(false, 'g', "requires the tool to generate temporary SVG files first and then us those files to generate the actual target");

	/** Application option to manage layers manually when creating a temporary directory. */
	protected final AO_ManualLayers optionManualLayers = new AO_ManualLayers(false, 'm', "layers are switched off/on on a raw text file, i.e. not using any SVG or XML library");
	...
	public int executeApplication(final String[] args) {
		...
		try{
			...
			if(!this.getMiscOptions().doesKeepTempArtifacts()){
